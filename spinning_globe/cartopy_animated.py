...
def update(i):
    ...
    print(label)
    fig.clf()
    ax = fig.add_axes([0, 0, 1, 1], projection=ccrs.Orthographic(central_longitude=i, central_latitude=20))
    ax.coastlines()
    add_logo(ax)
    return [ax]
...
if __name__ == '__main__':
    ...
    anim = FuncAnimation(fig, update, frames=np.linspace(0., 360, fps * animation_length, endpoint=False), interval=1000 // fps)
